...
class FirElementSerializer private constructor(
    ...
    fun propertyProto(property: FirProperty): ProtoBuf.Property.Builder? {
        ...
        val defaultAccessorFlags = Flags.getAccessorFlags(
            ...
            ProtoEnumFlags.modality(property.modality ?: Modality.SEALED),
            false, false, false
        )
        ...
        val flags = Flags.getPropertyFlags(
            hasAnnotations,
            ProtoEnumFlags.visibility(normalizeVisibility(property)),
            ProtoEnumFlags.modality(property.modality ?: Modality.FINAL),
    ...
    internal fun typeProto(typeRef: FirTypeRef): ProtoBuf.Type.Builder {
        if (typeRef !is FirResolvedTypeRef) {
            return ProtoBuf.Type.newBuilder() // TODO: serializeErrorType?
        }
...
//        }
    ...
    private fun getAccessorFlags(accessor: FirPropertyAccessor): Int = Flags.getAccessorFlags(
        accessor.annotations.isNotEmpty(),
        ProtoEnumFlags.visibility(normalizeVisibility(accessor)),
        ProtoEnumFlags.modality(accessor.modality ?: Modality.FINAL),
    ...
    private fun MutableVersionRequirementTable.serializeVersionRequirements(declaration: FirMemberDeclaration): List<Int> =
        serializeVersionRequirements(declaration.annotations)
